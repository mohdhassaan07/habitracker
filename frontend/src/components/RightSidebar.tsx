    ...
    const fetchLoggedData = async () => {
      try {
        ...
        if (res.status === 200) {
          ...
          const filtered = datesData
            .filter((log: any) => log.totalValue > 0)
            .map((log: any) => ({
              date: formatDateToDisplayString(new Date(log.date)).slice(0, 6),
              value: log.totalValue
            }));

          const recentFiltered = filtered.slice(filtered.length > 6 ? filtered.length - 6 : 0);
